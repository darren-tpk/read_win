...
class OneSecUnit(object):
    ...
    Attributes:
        ...
        starttime (:class:`~obspy.core.utcdatetime.UTCDateTime`): Start time of the unit
        stream (:class:`~obspy.core.stream.Stream`): Stream containing data from unit
    """
...
def _get_starttime(date_bytes):
    ...
    Returns:
        starttime (:class:`~obspy.core.utcdatetime.UTCDateTime`): Start time of 1 s unit as UTCDateTime
    """
...
def _read_single_win_file(file_path):
    ...
    Returns:
        stream (:class:`~obspy.core.stream.Stream`): Obspy Stream object containing data from WIN file (not merged or trimmed)
    """
...
def read_win(list_file_paths, channel_table_path, network='SA', station='KUR', fill_value=None):
    ...
    Returns:
        stream (:class:`~obspy.core.stream.Stream`): Stream containing infrasound data
    """
    ...
    for tr in stream:
        ...
        tr.data *= \
        channel_table.loc[channel_table['location'] == str(tr.stats.location), 'amplitude_correction'].values[0]
...
def read_win_dir(starttime,
    ...
    :param file_interval (str): WIN data file interval (must be "minute" or "hour" or "day")
    :param merge_value (float): Input to stream.merge() to combine data read from separate WIN files
    ...
    # Read WIN and trim
    stream = read_win(file_paths, channel_table_path, network='SA', station='KUR', fill_value=merge_value)
...
if __name__ == '__main__':
    ...
    merge_value = None  # for stream.merge
